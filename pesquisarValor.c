int pesquisarValor(){
	...
	char * tabelaEscolhida = (char*)malloc(21*sizeof(char));											//Reserva memória para identificação da tabela escolhida
	if(tabelaEscolhida == NULL){
		printf("ERRO! Memória não alocada.");
		exit(0);
	}

	char * nomeAtributoEscolhido = (char*)malloc(21*sizeof(char));								//Reserva memória para identificação do atributo escolhida para pesquisar valor
	...
	/*--Define estrutura nova linha, que possui arrays
			que guardam os atributos da linha a partir dos
			tipos de dados*/
	...
	/*--Lista dos atributos da tabela solicitada--*/
	do{
		pont_tamanho_tabela = fopen(tamanho, "r");
		if (pont_tamanho_tabela == NULL){
			printf("ERRO! O arquivo de tamanhos não foi aberto.\n");
	...
	/*--Localização do nome do atributo da tabela solicidata--*/
	pont_tamanho_tabela = fopen(tamanho, "r");
	if (pont_tamanho_tabela == NULL){
		printf("ERRO! O arquivo de tamanhos não foi aberto.\n");
		exit(0);
	...
	if (tipoAtributoEscolhido != 4 && tipoAtributoEscolhido != 8){
		...
		/*--Será verificada a opção que foi escolhida pelo usuário,
				bem como o tipo de dados do mesmo. Daí, seŕa solicitado
				o valor e exibido os dados que correspondem a pesquisa--*/
